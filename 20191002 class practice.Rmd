...
disposableIncome %>% slice(28:43) %>% gather(contains("可"), key = "不同家庭", value = "所得") -> disposableIncome1 

filter(disposableIncome1 , X1 == 2003) -> 基期所得
...
金融風暴 %>%  mutate("年" = as.integer(X1)) %>% ggplot(aes(x = 年, y = 速率, color = 不同家庭 )) + scale_fill_discrete_qualitative(palette="myQualitative",nmax=5) + geom_line() -> finalplot
